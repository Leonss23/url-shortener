...
export const POST: APIRoute = async ({ request, locals: { runtime: { env } } }) => {
...
async function parseURL(request: Request): Promise<string | ApiError> {
  try {
    ...
    if (contentType?.includes('application/json')) {
      unparsedURL = await request.json()
        .then((v) => v.url)
    ...
    if (!unparsedURL || typeof unparsedURL != 'string') return new ApiError(400, 'Missing URL string')

    if (!URL.canParse(unparsedURL)) return new ApiError(400, 'Invalid URL')
