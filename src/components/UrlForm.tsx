...
export default function UrlForm() {
  ...
  async function handleSubmit(e: Event & { submitter: HTMLElement } & { currentTarget: HTMLFormElement; target: DOMElement }) {
    e.preventDefault()
  ...
  return (
    <>
      <div
        class="sm:text-xl md:text-2xl flex flex-col items-center container"
      >
        ...
        >
          ...
          <button
            class="p-2 bg-indigo-800 text-white hover:bg-indigo-700 active:bg-indigo-600 duration-50"
          >Shorten</button>
        </form >
        <div
...
async function registerURL(url: string) {
  try {
    ...
    if (typeof shortUrl != 'string') throw new Error('failed to parse url from response')
    return shortUrl
  } catch (error) {
    console.error({ error })
  }
